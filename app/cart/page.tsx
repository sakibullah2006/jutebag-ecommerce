...
const Cart = () => {
    ...
    const calculatePrice = (product: CartItem) => {
        ...
        if (product.variation_id) {
            // If the product has a variation, return the price of the selected variation
            ...
            // If the product does not have a variation, return the regular or sale price
            if (product.on_sale) {
    ...
    cartState.cartArray.map(item => {
        const selectedPrice = item.selectedVariation ? item.selectedVariation.sale_price ?? item.selectedVariation.regular_price : (item.sale_price ?? item.regular_price);
        totalCart += Number(selectedPrice) * item.quantity
    })
                                                ...
                                                <div className="item flex md:mt-7 md:pb-7 mt-5 pb-5 border-b border-line w-full" key={product.id}>
                                                    <div className="w-1/2">
                                                        <div className="flex items-center gap-6">
                                                            ...
                                                            <div>
                                                                <div className="text-title">{product.name}-{product.variation_id}</div>
                            ...
                            <div className="checkout-block bg-surface p-6 rounded-2xl">
                                ...
                                <div className="total-cart-block pt-4 pb-4 flex justify-between">
                                    ...
                                    <div className="heading5">{decodeHtmlEntities(currentCurrency?.symbol || '')}
                                        <span className="total-cart heading5">{Number(totalCart - discountCart + shipCart).toFixed(2)}</span>
                                    </div>
                                </div>
